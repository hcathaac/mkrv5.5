...
Equations
   ...
   budget_sec(sec)  budget for sector sec
...
budget_CMK..     sum(p, budget(p,'CMK')*X(p))   =l=  22312335;
budget_WMK..     sum(p, budget(p,'WMK')*X(p))   =l=    647220;
budget_sterea..  sum(p, budget(p,'STE')*X(p))   =l=   3480311;
budget_sec(sec).. sum(p$(sector(p) eq ord(sec)), sum(rg, budget(p,rg))*X(p)) =l= totbudg(sec);
...
FILE fx /c:\gams\proj_PAVETout.txt/ ;
fx.pw=10000;
put fx ;
