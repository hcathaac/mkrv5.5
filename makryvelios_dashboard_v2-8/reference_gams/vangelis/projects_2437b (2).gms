...
loop(GREY, budget(GREY,rg)=0.925*budget(GREY,rg));
...
* random generation of project scores from uniform distribution
    ...
    loop(p,
         z1= score(p,'1')+0.5*uniformint(-1,1);
         if(z1<0, z1=0); if(z1>5, z1=5);
         z2= score(p,'2')+0.5*uniformint(-1,1);
         if(z2<0, z2=0); if(z2>5, z2=5);
         z3= score(p,'3')+0.5*uniformint(-1,1);
