# Makryvelios Research Analytics & Econometrics Command Centre v5.8.1


Version 5.8.1 is a strictly additive intelligence upgrade over v5.8.0. The standalone Agentic Research Mode now has a single evidence-aware multi-turn research conversation with three engines: a smart deterministic semantic router requiring no AI/API, optional local Ollama AI requiring no API key, and the retained external user-key LLM route. Agent answers retrieve the specific computed rows and uploaded-PDF page evidence relevant to the exact question rather than returning generic templates. The 150-question generator is now data-aware offline and can optionally use Local/API AI in batches grounded in the schema, observed relationship leads and PDF evidence. No earlier feature is removed.
